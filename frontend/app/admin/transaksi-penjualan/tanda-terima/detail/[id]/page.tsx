...
export default function Page() {
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const [detailResponse, itemsResponse] = await Promise.all([
                ...
                api.get(`/tanda-terima/${tandaTerimaId}/items`),
            ]);
    ...
    const handleExportPdf = () => {
        ...
        doc.setFont("times", "normal");
        doc.setFontSize(10);
        doc.text(`No. PO: ${detail.no_po || "-"}`, 18, 52);
        doc.text(`Kepada: ${detail.sppg?.nama_sppg ?? "-"}`, 18, 59);
        doc.text(`Tanggal: ${formatTanggal(detail.tanggal)}`, 125, 59);
